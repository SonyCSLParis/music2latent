...
class EncoderDecoder:
    def __init__(self, load_path_inference=None, device=None, unbound_latents=False):
        download_model()
        if device is None:
            self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        else:
            self.device = device
        self.unbound_latents = unbound_latents
    ...
    def encode(self, wv_batch, max_waveform_length=None, max_batch_size=None, extract_features=False):
        '''
        ...
        Latents are in unbounded logit space, compatible with flow matching.
        '''
        if max_waveform_length is None:
            max_waveform_length = max_waveform_length_encode
        if max_batch_size is None:
            max_batch_size = max_batch_size_encode
        return self.encode_audio_inference(wv_batch, self, max_waveform_length, max_batch_size, device=self.device, extract_features=extract_features)
    
    def decode(self, latent, denoising_steps=1, max_waveform_length=None, max_batch_size=None):
        '''
        latent: tensor of latents to decode with shape [audio_channels, dim, length]
                Expected in unbounded logit space (as returned by encode).
        ...
        return self.decode_latent_inference(latent, self, max_waveform_length, max_batch_size, diffusion_steps=denoising_steps, device=self.device)

    # Encode audio sample for inference
    # Parameters:
    #   audio_path: path of audio sample or numpy array of waveform
    #   model: trained consistency model
    #   device: device to run the model on
    # Returns:
    #   latent: FM-compatible latent in unbounded logit space, shape [audio_channels, dim, latent_length]
    @torch.no_grad()
    def encode_audio_inference(self, audio_path, trainer, max_waveform_length_encode, max_batch_size_encode, device='cuda', extract_features=False):
        ...
        if extract_features:
            ...
            latent = latent / sigma_rescale if not self.unbound_latents else torch.atanh(latent.clamp(-(1-1e-3), (1-1e-3)))
            return latent


    # Decode latent representation for inference
    # Parameters:
    #   latent: FM-compatible latent in unbounded logit space, shape [audio_channels, dim, length]
    #   model: trained consistency model
    ...
    @torch.no_grad()
    def decode_latent_inference(self,latent, trainer, max_waveform_length_decode, max_batch_size_decode, diffusion_steps=1, device='cuda'):
        trainer.gen = trainer.gen.to(device)
        trainer.gen.eval()
        downscaling_factor = 2**freq_downsample_list.count(0)
        ...
        # check if latent tensor is on cpu, if so move to device
        ...
        # invert logit transform: unbounded R -> [-1, 1]
        latent = latent*sigma_rescale if not self.unbound_latents else torch.tanh(latent)
